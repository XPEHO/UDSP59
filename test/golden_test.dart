...
void main() {
  testGoldens('ModuleAccordionElement should look correct', (tester) async {
            ...
            const ProviderScope(
              child: ModuleAccordionPart(
                modulePart: ModulePart(
                  ...
                  image: "",
                  elements: [
                    ModulePartElement(text: "Subtitle 1", image: ""),
                    ModulePartElement(text: "Subtitle 2", image: ""),
                  ],
                ),
                index: 0,
              ),
    ...
    await screenMatchesGolden(tester, 'module_accordion_element_grid');
  });
  ...
  testGoldens('ModulesCarouselElement should look correct', (tester) async {
    ...
    await screenMatchesGolden(tester, 'modules_carousel_element');
  });
  ...
  testGoldens('PageHeader should look correct', (tester) async {
    final builder = GoldenBuilder.grid(
      ...
      widthToHeightRatio: 0.5,
      bgColor: const Color(0xFFECF0F8),
    )..addScenario('PageHeader', const PageHeader(pageTitle: "Page Title"));
    await tester.pumpWidgetBuilder(
      builder.build(),
      surfaceSize: const Size(300, 160),
    );
    await screenMatchesGolden(tester, 'page_header');
  });
  ...
  testGoldens('TitleHeader should look correct', (tester) async {
    final builder = GoldenBuilder.grid(
      ...
      widthToHeightRatio: 0.5,
      bgColor: const Color(0xFFECF0F8),
    )..addScenario('TitleHeader', const TitleHeader());
    await tester.pumpWidgetBuilder(
      builder.build(),
      surfaceSize: const Size(300, 150),
    );
    await screenMatchesGolden(tester, 'title_header');
  });
  ...
  testGoldens('UrlLinkedIcon should look correct', (tester) async {
    final builder = GoldenBuilder.grid(
      ...
      widthToHeightRatio: 0.5,
      bgColor: const Color(0xFFECF0F8),
    )..addScenario('UrlLinkedIcon', const UrlLinkedIcon(url: '', size: 30));
    await tester.pumpWidgetBuilder(
      builder.build(),
      surfaceSize: const Size(130, 100),
    );
    await screenMatchesGolden(tester, 'url_linked_icon');
  });
  ...
  testGoldens('TipsCardSwitcher should look correct', (tester) async {
    final builder = GoldenBuilder.grid(
      ...
      widthToHeightRatio: 0.5,
      bgColor: const Color(0xFFECF0F8),
    )..addScenario('TipsCardSwitcher', const TipsCard(tips: 'Test'));
    await tester.pumpWidgetBuilder(
      builder.build(),
      surfaceSize: const Size(160, 160),
    );
    await screenMatchesGolden(tester, 'tips_card_switcher');
  });
}
